...
% Fills an N×N grid with values 1..N² such that all rows, columns,
% and both diagonals sum to the magic sum S = N*(N²+1)/2.

magic_square_bt(N, Square) :-
    length(Square, N),
    maplist(length_(N), Square),      % create N×N matrix of variables
    append(Square, Vars),             % flatten matrix to a single list
    Max is N * N,
    numlist(1, Max, Domain),          % domain = [1..N²]
