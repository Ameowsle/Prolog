...
### 1. Backtracking, Row-by-Row with Early Pruning (`012NonogramRowByRow.pl`)
...
### 2. Constraint Logic Programming (`012NonogramCLP.pl`)
...
Both solvers find the same unique solution. Logical inferences are the
...
On this small instance both are effectively instant. The row-by-row solver does
least work because partial-column pruning rejects dead grids early. The CLP
model is the one built to scale: its `automaton/3` constraints propagate the
clues during search, so it stays practical on large grids where a plain
...
The same query works for `012NonogramCLP.pl`. To solve a different puzzle, pass
its clues directly, e.g.:
