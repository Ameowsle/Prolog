...
% sudoku(-Solution): Solution is the filled-in NxN grid
sudoku(Solution) :-
    puzzle(Solution),
    length(Solution, N), % determine grid size dynamically
    fill_grid(Solution, Solution, N, 1).

% fill_grid(+Rows, +Solution, +N, +RowIdx)
...
% fill_row(+Cells, +RowIdx, +ColIdx, +Solution, +N)
...
cage_candidates(RowIdx, ColIdx, Solution, N, CageCands) :-
    ...
    include(valid_cage_val(RemainingCells, RemainingSum, N), WithoutUsed, CageCands). % include goes through every value in the list WithoutUsed, tests it against the predicate, and stores the one where the predicate returns true in the CageCands list
